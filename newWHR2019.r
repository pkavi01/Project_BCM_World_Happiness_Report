...
#----- newWHR2019 data (dbo.newRaw_WHR_2019 - Project_BCM) -----
...
df_19.drop(['Happiness_Status','Lower_Confidence_Interval','Upper_Confidence_Interval','Dystopia'],axis=1,inplace=True) 
...
Happiness_Score = dict(type = 'choropleth', 
           locations = finaldf19['Country'],
           locationmode = 'country names',
           z = finaldf19['Happiness_Score'], 
           text = finaldf19['Country'],
           colorscale = 'Blues_',
...
world_map_19 = go.Figure(data = [Happiness_Score], layout=layout)
iplot(world_map_19)
...
df19_results = discover.discover(finaldf.drop(['target', 'target_n'],axis=1).sample(frac=1), classifier_overrides)
...
seabornInstance.heatmap(df19_results.pivot(index = 'target', columns = 'feature', values = 'Happiness_Score').fillna(1).loc[finaldf.drop(['target', 'target_n'],axis = 1).columns,finaldf.drop(['target', 'target_n'],axis = 1).columns], annot=True, center = 0, ax = ax[0], vmin = -1, vmax = 1, cmap = "Blues")
...
train_data_dm,test_data_dm = train_test_split(finaldf19,train_size = 0.8,random_state=3)
...
cv = float(format(cross_val_score(complex_model_1,finaldf19[independent_var],finaldf19['Happiness_Score'],cv=5).mean(),'.3f'))
r = evaluation.shape[0]
