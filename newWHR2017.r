...
#----- newWHR2017 data (dbo.newRaw_WHR_2017 - Project_BCM) -----
df_17 = pd.read_csv('newWHR2017.csv')
#df_17.describe()
#df_17.info()
usecols = ['Year','Country','Happiness_Score','Overall_Happiness_Rank','GDP_per_Capita','Family','Health','Freedom','Trust','Generosity']
df_17.drop(['Happiness_Status','Lower_Confidence_Interval','Upper_Confidence_Interval','Dystopia'],axis=1,inplace=True) 
df_17.columns = ['Year','Country','Happiness_Score','Overall_Happiness_Rank','GDP_per_Capita','Family','Health','Freedom','Trust','Generosity']

#df_17['Year'] = 2017 #add year column - (IF WE HAD TO ADD 'YEAR')
...
df_17["target"] = pd.qcut(df_17['Overall_Happiness_Rank'], len(target), labels=target)
df_17["target_n"] = pd.qcut(df_17['Overall_Happiness_Rank'], len(target), labels=target_n)


#COMBINING ALL DATA FILE TO finaldf17

# APPENDING ALL TOGETHER
finaldf17 = df_17.append([df_17,df_18,df_19])
# finaldf17.dropna(inplace = True)

#CHECKING FOR MISSING DATA
finaldf17.isnull().any()

# FILLING MISSING VALUES OF "TRUST" WITH ITS MEAN
finaldf17.Trust.fillna((finaldf17.Trust.mean()), inplace = True)
finaldf17.head(10)

# Statistical details can be seen using "describe()" function. 
# Defining an empty dataframe "DataFrame17". This dataframe includes Root Mean Squared Error (RMSE), R-squared, Adjusted R-squared, and mean of the R-squared values obtained by the k-Fold Cross-Validation, which are the essential metrics to compare different models. 
# Having an R-squared value closer to one and smaller RMSE means a better fit. 
# Filling this dataframe with the results.

evaluation = pd.DataFrame17({'Model':[],
                          'Details':[],
...
px.scatter(finaldf17, x="GDP_per_Capita", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 


# --- TRAIN THE DATA SET. ---

train_data, test_data = train_test_split(finaldf17, train_size = 0.8, random_state = 3)
lr = LinearRegression()
...
cv = float(format(cross_val_score(lr,finaldf17[['GDP_per_Capita']],finaldf17['Happiness_Score'],cv=5).mean(),'.3f'))
...
px.scatter(finaldf17, x="Family", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
# TRAIN THE DATA SET.
train_data, test_data = train_test_split(finaldf17, train_size = 0.8, random_state = 3)
lr = LinearRegression()
...
px.scatter(finaldf17, x="Health", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
# TRAIN THE DATA SET.
train_data, test_data = train_test_split(finaldf17, train_size = 0.8, random_state = 3)
lr = LinearRegression()
...
px.scatter(finaldf17, x="Freedom", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
...
px.scatter(finaldf17, x="Generosity", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
...
px.scatter(finaldf17, x="Trust", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
...
df1 = finaldf17[['GDP_per_Capita', 'Family', 'Health', 'Freedom','Generosity', 'Trust']]
h = df1.hist(bins = 25, figsize = (17,17), xlabelsize = '10', ylabelsize = '10')
...
Happiness_Score = dict(type = 'choropleth', 
           locations = finaldf17['Country'],
           locationmode = 'country names',
           z = finaldf17['Happiness_Score'], 
           text = finaldf17['Country'],
           colorscale = 'Blues_',
...
world_map_17 = go.Figure(data = [Happiness_Score], layout=layout)
iplot(world_map_17)
...
df17_results = discover.discover(finaldf.drop(['target', 'target_n'],axis=1).sample(frac=1), classifier_overrides)
...
seabornInstance.heatmap(df17_results.pivot(index = 'target', columns = 'feature', values = 'Happiness_Score').fillna(1).loc[finaldf.drop(['target', 'target_n'],axis = 1).columns,finaldf.drop(['target', 'target_n'],axis = 1).columns], annot=True, center = 0, ax = ax[0], vmin = -1, vmax = 1, cmap = "Blues")
...
train_data_dm,test_data_dm = train_test_split(finaldf17,train_size = 0.8,random_state=3)
...
cv = float(format(cross_val_score(complex_model_1,finaldf17[independent_var],finaldf17['Happiness_Score'],cv=5).mean(),'.3f'))
r = evaluation.shape[0]
