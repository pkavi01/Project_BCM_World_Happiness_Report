...
#----- newWHR2018 data (dbo.newRaw_WHR_2018 - Project_BCM) -----
df_18 = pd.read_csv('newWHR2018.csv')
#df_18.describe()
#df_18.info()
usecols = ['Year','Country','Happiness_Score','Overall_Happiness_Rank','GDP_per_Capita','Family','Health','Freedom','Trust','Generosity']
df_18.drop(['Happiness_Status','Lower_Confidence_Interval','Upper_Confidence_Interval','Dystopia'],axis=1,inplace=True) 
df_18.columns = ['Year','Country','Happiness_Score','Overall_Happiness_Rank','GDP_per_Capita','Family','Health','Freedom','Trust','Generosity']

#df_18['Year'] = 2018 #add year column - (IF WE HAD TO ADD 'YEAR')
...
df_18["target"] = pd.qcut(df_18['Overall_Happiness_Rank'], len(target), labels=target)
df_18["target_n"] = pd.qcut(df_18['Overall_Happiness_Rank'], len(target), labels=target_n)


#COMBINING ALL DATA FILE TO finaldf18

# APPENDING ALL TOGETHER
finaldf18 = df_18.append([df_18,df_19])
# finaldf18.dropna(inplace = True)

#CHECKING FOR MISSING DATA
finaldf18.isnull().any()

# FILLING MISSING VALUES OF "TRUST" WITH ITS MEAN
finaldf18.Trust.fillna((finaldf18.Trust.mean()), inplace = True)
finaldf18.head(10)

# Statistical details can be seen using "describe()" function. 
# Defining an empty dataframe "DataFrame18". This dataframe includes Root Mean Squared Error (RMSE), R-squared, Adjusted R-squared, and mean of the R-squared values obtained by the k-Fold Cross-Validation, which are the essential metrics to compare different models. 
# Having an R-squared value closer to one and smaller RMSE means a better fit. 
# Filling this dataframe with the results.

evaluation = pd.DataFrame18({'Model':[],
                          'Details':[],
...
px.scatter(finaldf18, x="GDP_per_Capita", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 


# --- TRAIN THE DATA SET. ---

train_data, test_data = train_test_split(finaldf18, train_size = 0.8, random_state = 3)
lr = LinearRegression()
...
cv = float(format(cross_val_score(lr,finaldf18[['GDP_per_Capita']],finaldf18['Happiness_Score'],cv=5).mean(),'.3f'))
...
px.scatter(finaldf18, x="Family", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
# TRAIN THE DATA SET.
train_data, test_data = train_test_split(finaldf18, train_size = 0.8, random_state = 3)
lr = LinearRegression()
...
px.scatter(finaldf18, x="Health", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
# TRAIN THE DATA SET.
train_data, test_data = train_test_split(finaldf18, train_size = 0.8, random_state = 3)
lr = LinearRegression()
...
px.scatter(finaldf18, x="Freedom", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
...
px.scatter(finaldf18, x="Generosity", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
...
px.scatter(finaldf18, x="Trust", y="Happiness_Score", animation_frame="Year", animation_group="Country", size="Overall_Happiness_Rank", color="Country", hover_name="Country", trendline= "ols") 
...
df1 = finaldf18[['GDP_per_Capita', 'Family', 'Health', 'Freedom','Generosity', 'Trust']]
h = df1.hist(bins = 25, figsize = (18,18), xlabelsize = '10', ylabelsize = '10')
...
Happiness_Score = dict(type = 'choropleth', 
           locations = finaldf18['Country'],
           locationmode = 'country names',
           z = finaldf18['Happiness_Score'], 
           text = finaldf18['Country'],
           colorscale = 'Blues_',
...
world_map_18 = go.Figure(data = [Happiness_Score], layout=layout)
iplot(world_map_18)
...
df18_results = discover.discover(finaldf.drop(['target', 'target_n'],axis=1).sample(frac=1), classifier_overrides)
...
seabornInstance.heatmap(df18_results.pivot(index = 'target', columns = 'feature', values = 'Happiness_Score').fillna(1).loc[finaldf.drop(['target', 'target_n'],axis = 1).columns,finaldf.drop(['target', 'target_n'],axis = 1).columns], annot=True, center = 0, ax = ax[0], vmin = -1, vmax = 1, cmap = "Blues")
...
train_data_dm,test_data_dm = train_test_split(finaldf18,train_size = 0.8,random_state=3)
...
cv = float(format(cross_val_score(complex_model_1,finaldf18[independent_var],finaldf18['Happiness_Score'],cv=5).mean(),'.3f'))
r = evaluation.shape[0]
